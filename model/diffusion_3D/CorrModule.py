...
class RegHead(nn.Module):
    def __init__(self, in_channels=2, out_channels=2, kernel_size=3, stride=1, padding=1):
        super().__init__()
...
class SpatialTransformer_block(nn.Module):
    ...
    def forward(self, src, flow):
        ...
        # 检查flow的通道数，如果不是2，需要处理
        if flow.shape[1] != 2:
            print(f"Warning: flow has {flow.shape[1]} channels, expected 2. Taking first 2 channels.")
            flow = flow[:, :2, :, :]
