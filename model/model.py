...
class DDPM(BaseModel):
    ...
    def optimize_parameters(self):
        if self.opt[self.opt['phase']]["amp"]:
            ...
            # l_tot = loss
            diff_loss, sim_loss, reg_loss, fusion_loss, total_loss = loss
            total_loss.backward()
            self.optG.step()
        # set log
        ...
        # self.log_dict['l_flow'] = flow_loss.item()  # 添加变形场损失的日志
        self.log_dict['l_fus'] = fusion_loss.item()
        self.log_dict['l_tot'] = total_loss.item()
        

        # # 在每个训练周期结束后复制分类器参数到模态判别器
        # if isinstance(self.netG, nn.DataParallel):
        #     self.netG.module.denoise_fn.model_transfer.modal_dis.load_state_dict(
        #     self.netG.module.denoise_fn.model_transfer.classifier.state_dict())
        # else:
        #     self.netG.denoise_fn.model_transfer.modal_dis.load_state_dict(
        #     self.netG.denoise_fn.model_transfer.classifier.state_dict())
    ...
    def test_registration(self):
        ...
        with torch.no_grad():
            if self.opt[self.opt['phase']]["amp"]:
                ...
                input = torch.cat([self.data['M'], self.data['F']], dim=1)
                if isinstance(self.netG, nn.DataParallel):
                    self.out_M, self.flow, self_contD, self.contF, self.fusion_img = self.netG.module.registration(input)
                else:
                    self.out_M, self.flow, self.contD, self.contF, self.fusion_img = self.netG.registration(input)
        self.netG.train()
    ...
    def get_current_registration(self):
        ...
        out_dict['fusion_img'] = self.fusion_img.detach()
